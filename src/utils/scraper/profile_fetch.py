...
def _load_weapon_map() -> dict:
    ...
    d = json.load(open(os.path.join(DATA_DIR, "parsed_nikke.json"), encoding="utf-8"))
...
def _load_cube_name_map() -> dict:
    ...
    d = json.load(open(os.path.join(DATA_DIR, "cube.json"),
                       encoding="utf-8"))
    return {v["id"]: name for name, v in d.items() if isinstance(v, dict) and "id" in v}


def _load_equip_skill_table() -> dict:
    d = json.load(open(os.path.join(DATA_DIR, "equipment_skills.json"),
                       encoding="utf-8"))
