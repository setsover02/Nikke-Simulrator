...
export interface DamageParams {
   ...
   isNormalAttack?: boolean; // 일반 공격 여부 (소장품 일반공격 배율 적용 제어)
   ...
   critBonusBase: number; // 기본 크리 보너스 (일반적으로 0.5)
   ...
   weakPointBase: number; // 원소 코드 기본 보너스 (1.1)
   weakPointExtra: number; // 추가 원소 데미지 소스
   ...
   atkDmgUp: number; // 공격 데미지 증가 (Attack Damage)
   dotDmgUp: number; // 지속 데미지 증가 (Sustained Damage)
   pierceDmgUp: number; // 관통 데미지 증가 (Pierce Damage)
   partDmgUp: number; // 파츠 데미지 증가 (Damage To Parts)
   ignoreDefDmgUp: number; // 방어력 무시 (True Damage)
   projectileDmgUp: number; // 투사체 데미지 증가
   interruptionPartDmgUp: number; // 저지 파츠 데미지 증가 (Damage to Interruption Parts)
   extraDmgUp: number; // 기타 데미지 증가
   ...
   enemyTakenUp: number; // 적 받는 데미지 증가 (Damage Taken)
   shareDmgUp: number; // 분배 데미지 증가 (Distributed Damage)
   enemyTakenDown: number; // 적 받는 데미지 감소
}
