...
for (const dir of charDirs) {
  ...
  for (const f of files) {
    ...
    let canonical = ALIASES[f.replace('.json', '')] || ALIASES[fileBase];
    if (!canonical) {
      if (parsedSkills[rawName]) canonical = rawName;
      else if (parsedKeyMap[norm(rawName)]) canonical = parsedKeyMap[norm(rawName)];
      else if (parsedKeyMap[norm(fileBase)]) canonical = parsedKeyMap[norm(fileBase)];
    }
