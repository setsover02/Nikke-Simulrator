...
function createChar(data, slot) {
    const char = applyBaseStats(data, true, undefined, 'None', 0, slot, { skill1Level: 10, skill2Level: 10, burstLevelSkill: 10 });
    char.atk = 60000;
    char.defense = 4000;
    char.hp = 1000000;
    return char;
...
console.log('=== TEST 3: 우월코드 override 검증 (약점속성 작열[풍압보스] vs 약점속성 철갑[전격보스] vs 약점속성 수냉[작열보스] vs 약점속성 풍압[철갑보스]) ===');
for (const weakElem of ['작열', '철갑', '수냉', '풍압', '전격']) {
    ...
    const rapiTotal = result.log.filter(l => l.source === 'Char_16_0').reduce((sum, l) => sum + (l.value || 0), 0);
