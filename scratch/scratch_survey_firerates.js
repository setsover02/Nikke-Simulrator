...
console.log('=== 무기별 fireRate 현황 조사 ===\n');

for (const [weapon, list] of Object.entries(byWeapon)) {
    console.log(`\n【 무기: ${weapon} (총 ${list.length}명) 】`);
    const rateDist = {};
    list.forEach(item => {
    ...
    // 상이한 값들을 가진 캐릭터 목록 출력
    list.forEach(item => {
        console.log(`  - ${item.name.padEnd(20)} (${item.file.padEnd(30)}): fireRate = ${item.fireRate}, chargeTime = ${item.chargeTime}`);
    });
}
