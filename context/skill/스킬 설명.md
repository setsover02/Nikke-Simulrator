...
- single_stage: 특정 단계에서만 단독 발동 여부를 정의한다. (true / false)
- unit: 조건 값의 단위를 정의한다. (percent, count 등)
...
### effects
하나의 트리거 조건 발동 시 여러 개의 하위 효과를 동시에 적용해야 하는 경우, effect 객체 내부에 하위 effect 객체들의 배열로 정의한다. (예: 크라운 스킬 2 등)

### hits
다단 히트 공격 효과의 타격 횟수를 지정한다. (예: 리틀 머메이드 등)

### target_skill
효과의 변경 또는 조건 적용 대상이 되는 스킬 ID를 지정한다. (예: 홍련 : 흑영의 `target_skill: "skill_1"`)

### counts
효과 발동을 위한 횟수 조건 배열을 지정한다. (예: 홍련 : 흑영의 `counts: [1, 2, 3]`)

### count_limit
전투 중 해당 효과가 발동 가능한 최대 횟수이다. (예: `[전투 중 1회 발동]`의 경우 `count_limit: 1`)

### weapon_override
`change_weapon` 효과 발동 시 변경되는 무기의 상세 스펙 객체이다.
- `chargeTime`: 변경된 차지 시간 (초)
- `atkCoef`: 변경된 기본 공격력 계수 (% 배열 또는 숫자)
- `fullChargeDamage`: 변경된 풀 차지 대미지 배율 (%)
- `maxAmmo`: 변경된 최대 장탄 수 ("infinity" 또는 숫자/배열)
