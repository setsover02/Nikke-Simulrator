...
> `_based`가 붙는 경우 해당 버프의 시전자가 기준이 된다. 즉 hp_caster_based_pct는 해당 버프(스킬)을 시전한 시전 니케를 기준으로 한다. 즉 A 니케의 hp_caster_based_pct가 B에게 적용된경우 A의 체력 기준으로 적용한다.
> 그 외의 버프들은 모두 적용 대상의 기준을 따른다. 즉 A 니케가 atk_pct 버프를 B 니케에게 주었을 경우 B니케의 공격력 기준으로 증가한다.
